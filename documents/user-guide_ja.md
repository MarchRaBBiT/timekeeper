...
- **編集機能**: ユーザー詳細ドロワーの「編集」ボタンから、氏名・メールアドレス・ロール・システム管理者権限を変更できます（`PUT /api/admin/users/:id`）。
...
- **ログアウト**: ヘッダーの **ログアウト** ボタンから `POST /api/auth/logout` を呼び出します。
- **プロフィール更新**: 設定画面（`/settings`）から自分の氏名とメールアドレスを変更できます（`PUT /api/auth/me`）。
- **パスワード変更**: 設定画面で現在のパスワードと新しいパスワードを入力して変更します（最低 8 文字）。
- **パスワードリセット**: ログイン画面の「パスワードを忘れた場合」からメールアドレスを入力すると、リセット用リンクが送信されます。
