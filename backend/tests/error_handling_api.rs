...
use serde_json::json;
use timekeeper_backend::{middleware, models::user::UserRole, state::AppState};
...
async fn test_invalid_json_payload_returns_bad_request() {
    ...
    let app = Router::new()
        .route(
            "/api/requests/leave",
            axum::routing::post(timekeeper_backend::handlers::requests::create_leave_request),
        )
...
async fn test_invalid_uuid_format_returns_bad_request() {
    ...
    let app = Router::new()
        .route(
            "/api/admin/users/{id}",
            axum::routing::put(timekeeper_backend::handlers::admin::users::update_user),
        )
...
async fn test_out_of_range_pagination_params() {
    ...
    let app = Router::new()
        .route(
            ...
            axum::routing::get(timekeeper_backend::handlers::admin::audit_logs::list_audit_logs),
        )
        .layer(Extension(sysadmin))
...
async fn test_empty_authorization_header_returns_unauthorized() {
    ...
    let app = Router::new()
        .route(
            "/api/attendance/me",
            axum::routing::get(timekeeper_backend::handlers::attendance::get_my_attendance),
        )
...
async fn test_expired_jwt_token_returns_unauthorized() {
    ...
    insert_active_access_token(&pool, &access_token)
        .await
