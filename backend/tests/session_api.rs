...
use timekeeper_backend::{
    handlers::{admin::sessions as admin_sessions, sessions},
...
fn test_router_admin_sessions(pool: PgPool, user: User, claims: Claims) -> Router {
    ...
    Router::new()
        .route(
            ...
            axum::routing::get(admin_sessions::list_user_sessions),
        )
        .route(
            "/api/admin/sessions/{id}",
            axum::routing::delete(admin_sessions::revoke_session),
        )
        .layer(Extension(user))
        .layer(Extension(claims))
        .layer(Extension(RequestId("test-request-id".to_string())))
...
async fn test_revoke_other_user_session_fails() {
    ...
    let request = Request::builder()
        ...
        .header(
            ...
            format!(
                "Bearer {}",
                create_test_token(employee1.id, employee1.role.clone())
            ),
        )
        .body(Body::empty())
        .unwrap();
