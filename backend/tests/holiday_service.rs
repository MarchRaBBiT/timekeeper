...
async fn exception_can_override_weekly_holiday(pool: PgPool) -> sqlx::Result<()> {
    ...
    assert!(!decision.is_holiday);
    assert_eq!(decision.reason, HolidayReason::WorkingDayOverride);
    Ok(())
}
...
#[sqlx::test(migrations = "./migrations")]
async fn monthly_listing_includes_working_day_overrides(pool: PgPool) -> sqlx::Result<()> {
    let wed = NaiveDate::from_ymd_opt(2025, 1, 8).unwrap();
    let user_id = Uuid::new_v4().to_string();
    ...
    assert!(entries
        .iter()
        .any(|entry| entry.date == wed && entry.reason == HolidayReason::WorkingDayOverride));
