...
pub fn admin_routes(state: AppState) -> Router<AppState> {
    Router::new()
        .route(
            ...
            get(admin::list_attendance_correction_requests),
        )
        .route(
            "/api/admin/attendance-corrections/{id}",
            get(admin::get_attendance_correction_request_detail),
        )
        .route(
            "/api/admin/attendance-corrections/{id}/approve",
            put(admin::approve_attendance_correction_request),
        )
        .route(
            "/api/admin/attendance-corrections/{id}/reject",
            put(admin::reject_attendance_correction_request),
        )
        .route("/api/admin/attendance", get(admin::get_all_attendance))
        .route_layer(axum_middleware::from_fn_with_state(
            state.clone(),
            user_rate_limit,
        ))
...
pub fn system_admin_routes(state: AppState) -> Router<AppState> {
    Router::new()
        .route("/api/admin/attendance", put(admin::upsert_attendance))
        .route(
            "/api/admin/breaks/{id}/force-end",
            put(admin::force_end_break),
        )
        .route_layer(axum_middleware::from_fn_with_state(
            state.clone(),
            user_rate_limit,
...
mod tests {
    ...
    #[tokio::test]
    async fn user_attendance_routes_require_auth() {
        ...
        let request = Request::builder()
            ...
            .body(Body::empty())
            .expect("build attendance route request");
        let response = app.call(request).await.expect("call attendance route");

        assert_eq!(response.status(), axum::http::StatusCode::UNAUTHORIZED);
    }

    #[tokio::test]
    async fn admin_attendance_routes_require_admin_auth() {
        let state = test_state();
        let mut app = Router::new()
        ...
        let request = Request::builder()
            ...
            .body(Body::empty())
            .expect("build admin attendance route request");
        let response = app
            .call(request)
            .await
            .expect("call admin attendance route");

        assert_eq!(response.status(), axum::http::StatusCode::UNAUTHORIZED);
    }

    #[tokio::test]
    async fn system_admin_attendance_routes_require_system_admin_auth() {
        let state = test_state();
        let mut app = Router::new()
        ...
        let request = Request::builder()
            ...
            .uri("/api/admin/attendance")
            .body(Body::empty())
            .expect("build system admin attendance route request");
        let response = app
            .call(request)
            .await
            .expect("call system admin attendance route");
