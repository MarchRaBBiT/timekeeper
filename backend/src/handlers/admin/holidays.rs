...
use super::common::{DEFAULT_PAGE, DEFAULT_PER_PAGE, MAX_PAGE, MAX_PER_PAGE, bad_request, parse_optional_date, push_clause};

pub use self::query_validation::{validate_admin_holiday_query, AdminHolidayQueryParams};
...
/// List holidays with filtering and pagination
...
pub async fn create_weekly_holiday(
    ...
    if let Some(ends_on) = payload.ends_on {
        if ends_on < payload.starts_on {
            return Err((
                StatusCode::BAD_REQUEST,
                Json(json!({"error":"End date must be on or after start date"})),
            ));
        }
    }
