...
| `/api/requests/overtime` | `POST` | User | Body `CreateOvertimeRequest { date, planned_hours, reason? }` | `200 OvertimeRequestResponse` | `400` validation, `500` create failure | 残業申請作成。Redis 通知 queue 有効時は承認可能な manager 宛てに `request_submitted` 通知 job を enqueue する（enqueue 失敗は申請作成を失敗させない。現状は worker 未実装のため件数上限 `10,000`〔`APPLICATION_NOTIFICATION_QUEUE_MAX_LEN`〕で古い job を破棄する暫定運用。consumer は [EP-20260731-application-notification-worker](../exec-plans/active/EP-20260731-application-notification-worker.md) で実装する） |
...
| `/api/holiday-work-requests` | `POST` | User | Body `SubmitHolidayWorkRequest { work_date, benefit, substitute_date?, compensatory_minutes?, reason }` | `200 HolidayWorkRequestResponse` | `400` benefitと付与項目の組合せ不正/同日振替/分数範囲外、`409` active申請重複 | 休日出勤申請を作成。`substitution`は別日の振替先、`compensatory`は1–1440分の付与量が必須 |
| `/api/holiday-work-requests/me` | `GET` | User | Query `status?` | `200 HolidayWorkRequestResponse[]` | `500` lookup failure | 本人の休日出勤申請一覧 |
| `/api/holiday-work-requests/{id}` | `DELETE` | User | Path `id` | `200 HolidayWorkRequestResponse` | `409` pending以外または本人以外 | 本人のpending申請を取消 |
...
| Endpoint | Method | Auth | Parameters | Success Response | Primary Errors | Summary |
| --- | --- | --- | --- | --- | --- | --- |
| `/api/leave-balances/me` | `GET` | User | Query `leave_type_code=annual`, `as_of?` (`YYYY-MM-DD`, 省略時はサーバー現在日) | `200 LeaveBalanceResponse { user_id, leave_type, as_of, available_minutes, available_days, active_lots, upcoming_expiries, obligations }` | `400` unknown/non-balance-tracked type, `500` ledger/rule lookup failure | 本人の残高連動休暇を種別別に ledger から導出する。`leave_type_code` 省略時は後方互換で `annual`。義務ルールがある種別のみ取得義務の進捗を返し、消化・引当は行わない |
| `/api/admin/users/{user_id}/leave-balances` | `GET` | Manager+ | Path `user_id`; Query `leave_type_code=annual`, `as_of?` | `200 LeaveBalanceResponse` | `400` invalid user_id/unknown or non-balance-tracked type, `403` 部署スコープ外, `500` lookup failure | 配下ユーザーの残高連動休暇を種別別に返す。System Admin は全ユーザー、Manager は配下のみ |
| `/api/admin/leave-grants/run` | `POST` | System Admin | Body `LeaveGrantRunRequest { leave_type_code="annual", base_date, dry_run=false, user_ids?, exclude_user_ids? }` | `200 LeaveGrantRunResponse { base_date, dry_run, granted[], skipped[], expired[] }` | `400` invalid input/unknown or non-balance-tracked type/unknown explicit user, `409` 付与ルール未設定・付与バッチ実行中（二重起動）, `500` append/lookup failure | 管理者起動で指定種別の付与を実行する。`leave_type_code` 省略時は `annual`。`dry_run=true` は書き込まず排他も取らない。実書き込みはユーザー単位の transaction（users 行ロック + 台帳行 FOR UPDATE）で行い、1 ユーザーの失敗は他ユーザーへ波及せずログに記録される（失敗ユーザーは同じ base_date の再実行で再対象化）。バッチ全体は `leave_grant_batch_lock` の claim/release で二重起動を排他し、クラッシュ残留 claim は 600 秒で自動失効する。cron 常駐は不要 |
| `/api/admin/leave-ledger/adjust` | `POST` | System Admin | Body `LeaveLedgerAdjustRequest { user_id, leave_type_code="annual", amount_minutes, lot_id?, day_equivalent_minutes?, granted_at?, expires_at?, grant_base_date?, reason, dry_run=false }` | `200 LeaveLedgerAdjustResponse { dry_run, entry?, balance_after_minutes, balance_after_days }` | `400` unknown/non-balance-tracked type/reason 不足/日付不正/過剰控除/unknown lot, `404` user not found, `500` append/lookup failure | 初期移行・手動補正用に指定種別の append-only ledger へ `adjust` を投入する。`leave_type_code` 省略時は `annual`。新規ロット投入は正の分数と付与日・時効日が必須。`dry_run=true` で投入後残高を照合できる。実書き込みは users 行ロック + 台帳行 FOR UPDATE の同一 transaction で残高検証と追記を行う |
| `/api/admin/leave-types` | `GET` / `POST` | System Admin | POST Body `CreateLeaveTypeRequest { code, name, is_paid, balance_tracked, allowed_units[] }` | `200 LeaveTypeResponse[]` / `201 LeaveTypeResponse` | `400` invalid code/unit, `409` duplicate code | 会社固有の休暇種別を一覧・作成する。`allowed_units` は `day / half_am / half_pm / hour` |
| `/api/admin/leave-types/{code}` | `PUT` / `DELETE` | System Admin | PUT Body `UpdateLeaveTypeRequest { name, is_paid, balance_tracked, allowed_units[], is_active }` | `200 LeaveTypeResponse` / `204` | `404` unknown code, `409` legacy code または参照中 | 種別コードは不変。既存4種は削除不可で、停止は `is_active=false` を用いる |
...
| `/api/admin/holiday-work-requests` | `GET` | Manager+ | なし | `200 HolidayWorkRequestResponse[]` | `403` forbidden, `500` lookup failure | System Adminは全件、Managerは管理部署内のpending申請を返す |
| `/api/admin/holiday-work-requests/{id}/approve` | `POST` | Manager+ | Body `HolidayWorkDecision { comment }` | `200 HolidayWorkRequestResponse` | `400` day-kind/設定不正、`403` scope外/自己承認、`409` pending以外・resolved未生成・片側locked | 振替pair overrideまたはsource一意なcompensatory grantを単一transactionで確定 |
| `/api/admin/holiday-work-requests/{id}/reject` | `POST` | Manager+ | Body `HolidayWorkDecision { comment }` | `200 HolidayWorkRequestResponse` | `403` scope外/自己却下、`409` pending以外 | 休日出勤申請を却下 |
...
| `/api/admin/work-schedule-anomalies` | `GET` | Manager+ | Query `user_id?`, `from`, `to` | `200 WorkScheduleAnomalyListResponse` | `400 INVALID_WORK_SCHEDULE`, `403` scope外 | 未設定、予定外勤務、打刻漏れ、休暇日打刻（`leave_conflict`）、未申請/超過残業、遅刻、早退、欠勤、休憩不足、勤務間休息不足（`insufficient_rest`）の anomaly を範囲検出する。`insufficient_rest` は検索範囲直前の完了勤務も参照し、補正後のeffective clock-outから次のeffective clock-inまでの実timestamp差が設定値（既定660分）未満の場合にだけ生成する。夜勤・日跨ぎも暦日で推測せず、検出によって打刻を拒否しない。承認済み休暇のみの日は打刻漏れ・欠勤扱いにしない。残業判定（`unapproved_overtime`/`overtime_exceeds_request`）は実働時間（clock span − 休憩実績）と`expected_work_minutes`（休憩控除後の所定時間）の差分で計算し、`schedule_type = fixed`の日のみが対象（flexの日は対象外。flexの`expected_work_minutes`はflex band幅であり契約所定時間ではないため）。`absent`（打刻なしの過去日）と`missing_clock_in`（打刻なしの当日以降）の境界は業務タイムゾーン（`state.config.time_zone`、既定 Asia/Tokyo）の「今日」基準で判定する（UTC日付ではない） |
| `/api/admin/attendance-report` | `GET` | Scoped Manager+ | Query `year`, `month`, `department_id?`, `page?`, `per_page?` | `200 AdminAttendanceReportResponse` | `400` query不正、`403` scope外department | T-03月次区分、T-08 anomaly、T-09 36協定状態を合成した月次一覧。read-onlyでprojectionを生成せず、scope/月単位のset-based preloadを使う。resolved workday不足は`unresolved_days`として返し、計算不能区分を0へ潰さない。全対象を`exceeded > warning > ok`、次にuser IDでsortしてからpaginationする |
...
| `/api/admin/overtime-monitor/settings` | `GET` / `PUT` | System Admin | `GET`: なし; `PUT`: Body `OvertimeMonitorSettingsRequest` | `200 OvertimeMonitorSettingsResponse` | `400 INVALID_WORK_SCHEDULE`, `403` forbidden, `500` persistence failure | 36協定監視と勤務間休息の effective-dated 閾値設定を取得/更新する。年度開始月、月次/年次/平均上限、warning 比率、残業申請突合の許容差、任意の`minimum_rest_minutes`（省略時は新規行660分、既存行更新では現在値を維持）を持つ。DTO側は分単位フィールドが`i64`だがDBカラムは`INTEGER`（`i32`）のため、`monthly_limit_minutes`/`yearly_limit_minutes`/`rolling_average_limit_minutes`/`single_month_absolute_limit_minutes`は`5,270,400`分（10年分）、`overtime_request_tolerance_minutes`と`minimum_rest_minutes`は`44,640`分（31日分）を超えると`400 INVALID_WORK_SCHEDULE`を返す |
...
| `/api/admin/payroll-export` | `GET` | System Admin | Query `year`, `month` | `200 PayrollExportResponse { exported, failed, csv }` | `400` invalid period, `403`, `500` persistence failure | closed workflowだけをimmutable snapshot revisionから一括出力。未締め・reopenedはHTTP 200内の`failed[].code=monthly_not_closed`。CSVはUTF-8 BOM、CRLF、固定列、整数分 |
