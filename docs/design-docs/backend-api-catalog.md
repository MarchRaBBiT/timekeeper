...
| `/api/admin/work-schedules/{id}/versions/{version_id}` | `PUT` | System Admin | Body `ReplaceWorkScheduleVersionRequest { revision, ..., schedule_type? = "fixed", flex_policy? }` | `200 WorkScheduleVersionResponse` | `409 REVISION_CONFLICT` / `PUBLISHED_VERSION_IMMUTABLE`, `422` interval不正 | 楽観ロック付きでdraft版全体を置換する |
...
| `/api/admin/users/{user_id}/work-schedule-calendar` | `GET` | Scoped Manager+ | Path `user_id`; Query `from`, `to` | `200 WorkScheduleCalendarResponse` | `400 INVALID_WORK_SCHEDULE`, `403` scope外 | resolved workday、attendance、anomalyを日別カレンダーとして返す |
