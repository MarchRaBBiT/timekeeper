...
| `/api/admin/requests/{id}/approve` | `PUT` | Admin | Path `id`; Body `ApprovePayload { comment }` | `200 {"message":"Request approved"}` | `400` comment 不正、annual 残高不足 `LEAVE_BALANCE_INSUFFICIENT`, 稼働日 0 日の期間は `LEAVE_REQUEST_NO_WORKING_DAYS`, アクティブなロットが無い場合は `LEAVE_REQUEST_NO_ACTIVE_LOT`, `403` self-approval 禁止, `404` not found/already processed, `422` 申請期間内の勤務予定を解決できない日を含む場合 `LEAVE_REQUEST_SCHEDULE_UNRESOLVED`（fail-closed）, `500` update/ledger append failure | leave / overtime 申請承認。`annual` は承認時に稼働日数を申請時と同じロジックで再解決し、残高を再検証したうえで同一 transaction で FIFO `consume` を ledger へ追記する（消化対象日は稼働日のみ） |
