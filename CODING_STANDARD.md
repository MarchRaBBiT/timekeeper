...
リファクタリング・機能追加を問わず、以下を必須ルールとする。

- **リファクタリング前後テスト必須**: 対象箇所の変更前後で、挙動を保証する関連テストを実行すること。
- **テスト未整備時の先行実装**: 対象箇所にテストがない場合、まずテストを実装し、その成功を確認してから本体変更を行うこと。
- **機能実装前のテスト先行**: 新機能では「あるべき挙動」を表すテストを先に実装し、その後に実装を行うこと。
- **完了条件**: 変更対象に関連するテストが成功していることを、作業完了の必須条件とする。

**推奨フロー（最小）**:
1. 期待挙動をテストで定義
2. テストを失敗させて妥当性確認（新機能時）
3. 本体実装
4. 関連テストを再実行して成功確認
